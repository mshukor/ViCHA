...
class ViCHA(nn.Module):
    def __init__(self,                 
        ...
        self.return_attention = config.get('return_attention', None)
        ...
        if self.return_hidden_state:
            print('self.ita_hidden_weights', self.ita_hidden_weights)
    ...
    def forward(self, image, text, alpha=0):
        ...
        if self.return_hidden_state:
            ...
            image_embeds = self.visual_encoder(image, register_blk=self.register_blk_id) 
        ...
        with torch.no_grad():
            ...
            if self.return_hidden_state:
                image_embeds_m, image_hidden_states_m = self.visual_encoder_m(image)
            else:
            ...
            sim_targets_i2t = sim_targets
            sim_targets_t2i = sim_targets

            sim_i2t_targets = alpha * F.softmax(sim_i2t_m, dim=1) + (1 - alpha) * sim_targets_i2t
        ...
        if self.return_hidden_state:
            image_hidden_states_m = None
            text_hidden_states_m = None

            loss_ita_hidden = self.hidden_states_ita(image_hidden_states[-(self.num_hidden_align+1): -1], text_output.hidden_states[-(self.num_hidden_align+1):-1], 
                                weights=self.ita_hidden_weights, 
                                image_hidden_states_m=image_hidden_states_m, text_hidden_states_m=text_hidden_states_m, alpha=alpha)
    ...
    def _dequeue_and_enqueue_ms(self, image_feat, text_feat, index=None):
        # gather keys before updating queue
        image_feats = concat_all_gather(image_feat)
        text_feats = concat_all_gather(text_feat)
        ...
        # replace the keys at ptr (dequeue and enqueue)
        self.image_queue_ms[index, :, ptr:ptr + batch_size] = image_feats.T
        self.text_queue_ms[index, :, ptr:ptr + batch_size] = text_feats.T
        # ptr = (ptr + batch_size) % self.queue_size  # move pointer
    ...
    def hidden_states_ita(self, image_hidden_states, text_hidden_states, weights=None, 
        image_hidden_states_m=None, text_hidden_states_m=None, alpha=None):
        loss_ita_hidden = 0
        if weights is None:
        ...
        for i in range(len(image_hidden_states)):
            ...
            loss_ita_hidden += weights[i]*(loss_ita_avg + (loss_i2t+loss_t2i)/2)
