...
from models.model_nlvr import ViCHA
...
from models.model_nlvr_kw_img import kw_img_ViCHA
...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i,(image0, image1, text, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if config.get('kw_img_dataset', False):
            text_inputs0 =tokenizer(text[0], padding='longest', return_tensors="pt").to(device) 
            text_inputs1 =tokenizer(text[1], padding='longest', return_tensors="pt").to(device)
            text_inputs2 =tokenizer(text[2], padding='longest', return_tensors="pt").to(device)  
            text_inputs = [text_inputs0, text_inputs1, text_inputs2]
        else:
...
def main(args, config):
    ...
    if config.get('kw_img_dataset', False):
        ...
        datasets = create_kw_img_dataset('nlvr', config, data_dir=args.data_dir)
    else:
    ...
    if args.checkpoint:    
        ...
        if config.get('kw_model', False):
            
            state_dict = model.initialize(state_dict)
